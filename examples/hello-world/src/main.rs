use polkavm::{Config, Engine, Module, ProgramBlob, Reg, InterruptKind, Linker};
...
fn main() {
    ...
    let result = instance.call_typed_and_get_result::<u32, (u32, u32)>(&mut (), "add_numbers", (1, 10)).unwrap();
    println!("  1 + 10 + 100 = {}", result);

    // Low-level API.
    let entry_point = module.exports().find(|export| export == "add_numbers").unwrap().program_counter();
    let mut instance = module.instantiate().unwrap();
    instance.set_program_counter(entry_point);
