...
unsafe fn clear_program() {
    ...
    if current.stack_size() > 0 {
        linux_raw::sys_munmap(current.stack_address_low() as *mut core::ffi::c_void, current.stack_size() as usize)
            .unwrap_or_else(|error| abort_with_error("failed to unmap user stack", error));

        current.clear_stack_size();
    }

    if current.code_size() > 0 {
        linux_raw::sys_munmap(VM_ADDR_NATIVE_CODE as *mut core::ffi::c_void, current.code_size())
            .unwrap_or_else(|error| abort_with_error("failed to unmap user code", error));
