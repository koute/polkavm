...
use crate::{RawInstance, Reg, Error, ProgramSymbol, ProgramCounter, InterruptKind, Module};
...
impl<UserError, E> ReturnTy<UserError> for Result<(), E> where UserError: From<E>, E: 'static {
    const _REGS_REQUIRED: usize = 0;
...
impl<UserData, UserError> Instance<UserData, UserError> {
    ...
    pub fn call_typed<FnArgs>(
        ...
        fn_name: &str,
        args: FnArgs
    ) -> Result<(), CallError<UserError>> where FnArgs: FuncArgs {
        let Some(&entry_point) = self.pre.0.exports.get(fn_name.as_bytes()) else {
            return Err(CallError::Error(Error::from_display(format!("export not found: '{fn_name}'"))));
        };

        self.instance.prepare_call_typed(entry_point, args);

        loop {
            match self.instance.run().map_err(CallError::Error)? {
                InterruptKind::Finished => break,
                InterruptKind::Segfault(..) | InterruptKind::Trap(..) => return Err(CallError::Trap),
    ...
    pub fn call_typed_and_get_result<FnResult, FnArgs>(
        ...
        fn_name: &str,
        args: FnArgs
    ) -> Result<FnResult, CallError<UserError>> where FnArgs: FuncArgs, FnResult: FuncResult {
        self.call_typed(user_data, fn_name, args)?;
