...
impl Tracer {
    ...
    pub fn step(&mut self, interruption: &InterruptKind) {
    ...
    pub fn set_reg(&mut self, reg: Reg, value: u32) {
        if let Some(ref mut interpreter) = self.crosscheck_interpreter {
            interpreter.set_reg(reg, value);
        }
    }

    pub fn set_program_counter(&mut self, pc: u32) {
        if let Some(ref mut interpreter) = self.crosscheck_interpreter {
            interpreter.set_program_counter(pc);
        }
    }

    pub fn write_memory(&mut self, address: u32, data: &[u8], success: bool) {
        if let Some(ref mut interpreter) = self.crosscheck_interpreter {
            let expected_result = interpreter.write_memory(address, data);
            ...
            if success != expected_result.is_ok() {
                ...
                panic!("Memory write in hostcall mismatch when writing to 0x{address:x}..0x{address_end:x}! (crosscheck interpreter = {expected_success}, actual = {success})");
            }
        }
    }

    pub fn sbrk(&mut self, size: u32) {
        if let Some(ref mut interpreter) = self.crosscheck_interpreter {
            interpreter.sbrk(size);
        }
    }
